 ...
 * A httpd server
 * 2017-12-12
 * V1.0 -- A simple version
 * 1. The worker should not use ET mode
 * 2. The distribute system can work well
 * 3. No http info, just a simple server for client
 */
    ...
    {   
        ...
        for(i=0; i<nfds; i++)
        {
            ...
            if((ok_fd == thread->pipe_fd) && (events[i].events & EPOLLIN))
            {
                ...
                while(1)
                {
                    const char *str = "Your are the client##";
                    char buf[32] = {0};
                    sprintf(buf, "%s%d", str, conn_fd);
