...
int http_head_info_analysis(char *ptr)
{
    assert(ptr != NULL);
    int len = strlen(ptr);
    char method[10];
    int i = 0, j = 0, is_get = 0, is_post = 0;

    //Step1: Get http method
    while((!ISspace(ptr[j])) && (i < sizeof(method) -1))
    {
        method[i] = ptr[j];
        i++;
        j++;
    ...
    if(strncmp(method, "GET", 3) == 0)
        ...
        return -1; //do not support yet

    //skip the space
    while((ISspace(ptr[j])) && (j < len -1))
        j++;

    //Step2: Get http request url
    char url[255];
    i = 0;
    while((!ISspace(ptr[j])) && (i < sizeof(url) -1) && (j < len-1))
    {
        url[i] = ptr[j];
        i++;
        j++;
    ...
    //Step3: now have know method and url,
    //we need to judge if it is a valid request
    //printf("Method: %s\n", method);
    printf("Url: %s\n", url);
    ...
    {   
        ...
        for(i=0; i<nfds; i++)
        {
            ...
            if((ok_fd == thread->pipe_fd) && (events[i].events & EPOLLIN))
            {
                ...
                if(strlen(buffer) == 0)
                {
                    close(conn_fd); //peer closed
                    continue;
                }
                int rc = 0;
                if((rc = http_head_info_analysis(buffer)) < 0)
                {
                    if(rc == -1)
                        unimplemented(ok_fd);
                    else if(rc == -2)
